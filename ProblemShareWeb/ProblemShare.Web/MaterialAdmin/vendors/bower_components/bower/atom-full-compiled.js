...
(function (Object, Array, undefined) {
    // Safari 5 bug:
    // 'use strict';
    var toString = Object.prototype.toString, hasOwn = Object.prototype.hasOwnProperty, slice = Array.prototype.slice, atom = this.atom = function () {
        if (atom.initialize)
    ...
    */
    ...
    if (!Function.prototype.bind) {
        Function.prototype.bind = function (context /*, arg1, arg2... */) {
            ...
            var args = slice.call(arguments, 1), toBind = this, Nop = function () { }, Bound = function () {
                var isInstance;
                ...
                return toBind.apply(isInstance ? this : (context || {}), args.concat(slice.call(arguments)));
            };
        ...
        var lookup = nonStandard ?
            ...
            function (from, key, bool) {
                ...
                if (!descriptor) {
                    ...
                    if (proto)
                        return atom.accessors.lookup(proto, key, bool);
                }
    ...
    */
    (function (window, document) {
        var regexp = {
            ...
            Class: /^\.[-_a-z0-9]+$/i,
            Id: /^#[-_a-z0-9]+$/i
        }, isArray = Array.isArray, prevent = function (e) {
            e.preventDefault();
            return false;
        }, ignoreCssPostfix = {
            zIndex: true,
            ...
            zoom: true,
            lineHeight: true
        }, domReady = false, onDomReady = [], camelCase = function (str) {
        ...
        Dom.prototype = {
            ...
            filter: function (selector) {
                ...
                return new Dom(this.elems.filter(function (elem) {
                    if (property) {
                        ...
                        return elem.parentNode && coreToArray(elem.parentNode.querySelectorAll(selector)).indexOf(elem) >= 0;
                    }
                }));
            },
            ...
            find: function (selector) {
                var result = [];
                this.each(function (elem) {
                    var i = 0, found = Dom.find(elem, selector), l = found.length;
                    while (i < l)
            ...
            hasClass: function (classNames) {
                ...
                this.each(function (elem) {
                    if (result)
                        return;
                    var i = classNames.length, all = elem.className.split(/\s+/);
                    while (i--)
                        if (!coreContains(all, classNames[i])) {
                            return;
                        }
                    result = true;
                });
    ...
    new function () {
        function innerExtend(proto) {
            return function (elem, from) {
                ...
                var ext = proto ? elem.prototype : elem, accessors = atom.accessors && atom.accessors.inherit;
                for (var i in from)
                    if (i != 'constructor') {
                        if (accessors && accessors(from, ext, i))
                            continue;
                        ext[i] = clone(from[i]);
                    }
                return elem;
        ...
        function typeOf(item) {
            ...
            if (item && type == 'object') {
                if (atom.Class && item instanceof atom.Class)
                    return 'class';
                if (coreIsArrayLike(item))
                    return 'arguments';
            }
            return type;
        }
        ...
        clone.types = {
            ...
            'object': function (object) {
                ...
                for (var key in object) {
                    if (accessors && accessors(object, c, key))
                        continue;
    ...
    */
    atom.dom.prototype.ajax = function (config) {
        ...
        if (config.onError)
            config.onError = config.onError.bind($dom);
        atom.ajax(config);
        return $dom;
    };
    /*
    ...
    */
    (function () {
        var previous, started = false, callbacks = [], remove = [], frameTime = 16, // 62 fps
    ...
    */
    (function (document) {
        var prefix = 'pointerLockElement' in document ? '' :
    ...
    */
    new function () {
        var uri = function (str) {
            var o = atom.uri.options, m = o.parser.exec(str || window.location.href), uri = {}, i = 14;
            while (i--)
                uri[o.key[i]] = m[i] || "";
            uri[o.q.name] = {};
    ...
    */
    (function (atom) {
        var typeOf = atom.core.typeOf, extend = atom.core.extend, accessors = atom.accessors.inherit, prototype = 'prototype', lambda = function (value) { return function () { return value; }; }, prototyping = false;
        var Class = function (params) {
            if (prototyping)
                return this;
            if (typeof params == 'function' && typeOf(params) == 'function')
    ...
    atom.Class.bindAll = function (object, methods) {
        if (typeof methods == 'string') {
            ...
            for (var i = methods.length; i--;)
                atom.Class.bindAll(object, methods[i]);
        }
        else {
            for (var i in object)
                atom.Class.bindAll(object, i);
        }
        return object;
    };
    /*
    ...
    */
    new function () {
        ...
        var initEvents = function (object, reset) {
            if (reset || !object._events)
                object._events = { $ready: {} };
        };
        ...
        coreAppend(Class, {
            Events: Class({
                addEvent: function (name, fn) {
                    ...
                    if (arguments.length == 1 && typeof name != 'string') {
                        ...
                        if (name == '$ready') {
                            ...
                            if (ready)
                                fire.apply(this, [name, fn, ready, onfinish]);
                            onfinish.invoke();
                        }
                    }
                    return this;
                ...
                fireEvent: function (name, args) {
                    ...
                    if (funcs) {
                        var l = funcs.length, i = 0;
                        for (; i < l; i++)
                            fire.call(this, name, funcs[i], args || []);
                    }
                    return this;
                },
    ...
    */
    new function () {
        ...
        atom.Class.Mutators.Generators.init = function (Class, properties) {
            for (var i in properties)
                atom.accessors.define(Class.prototype, i, { get: getter(i, properties[i]) });
        };
    };
    /*
    ---
    ...
    */
    var declare = (function (atom) {
        var declare, methods, accessors = atom.accessors.inherit, factory = false, prototyping = false, mutators = [];
        declare = function (declareName, Parent, params) {
            ...
            var Constructor = function () { return methods.construct.call(this, Constructor, arguments); };
            ...
            Constructor.prototype.constructor = Constructor;
            if (params.declareName)
                methods.define(params.declareName, Constructor);
            return Constructor;
        };
        ...
        methods = {
            ...
            prepareParams: function (declareName, Parent, params) {
                ...
                if (!params.name)
                    params.name = declareName;
                if (!params.declareName)
                    params.declareName = declareName;
        ...
        declare.config
            ...
            if (mixins)
                methods.mixin(Constructor, mixins);
        })
    ...
    */
    ...
    declare('atom.Events', {
        ...
        unlock: function (name) {
            var action, all = this.actions[name], index = this.locked.indexOf(name);
    ...
    */
    ...
    declare('atom.Settings', {
        ...
        get: function (name, defaultValue) {
            if (Array.isArray(name))
                return this.subset(name, defaultValue);
        ...
        exportProperty: function (i, values) {
            var target = this.propertiesTarget, names = this.propertiesNames;
    ...
    */
    atom.object = {
        ...
        deepEquals: function (first, second) {
            if (!first || (typeof first) !== (typeof second))
                return false;
            for (var i in first) {
                var f = first[i], s = second[i];
                if (typeof f === 'object') {
                    if (!s || !Object.deepEquals(f, s))
                        return false;
                }
                else if (f !== s) {
                    return false;
    ...
    */
    ...
    declare('atom.Animatable', {
        ...
        initialize: function (callbacks, context) {
            ...
            if (!callbacks)
                throw new TypeError('callbacks cant be null');
            this.animations = [];
    ...
    declare('atom.Animatable.Animation', {
            ...
            for (var i in delta)
                if (delta.hasOwnProperty(i)) {
                    if (progress == null) {
                        ...
                        animatable.set(i, atom.Color && target instanceof atom.Color
                            ? target.toString() : target);
                    }
                    else {
                        initial = this.initial[i];
                        animatable.set(i, atom.Color && initial instanceof atom.Color ?
        ...
        tick: function (time) {
            ...
            this.changeValues(this.transition(lastTick ? 1 : (this.allTime - this.timeLeft) / this.allTime));
    ...
    */
    ...
    /** @deprecated */
    {
        ...
        setOptions: function (options) {
            if (!this.settings) {
                this.settings = new atom.Settings(atom.clone(this.options || {}));
    ...
    declare('atom.Events.Mixin', new function () {
        var init = function () {
            ...
            if (!events)
                events = this.__events = new atom.Events(this);
            if (this._events) {
    ...
    declare('atom.Color', {
        ...
        isColorString: function (string) {
            if (typeof string != 'string')
                return false;
            return Boolean(string in this.colorNames ||
                string.match(/^#\w{3,6}$/) ||
                string.match(/^rgba?\([\d\., ]+\)$/));
        },
        colorNames: {
    ...
    */
    atom.declare('atom.ImagePreloader', {
        ...
        get info() {
            var stat = atom.string.substitute("Images loaded: {load}; Errors: {error}; Aborts: {abort}", this.count);
            if (this.isReady)
                stat = "Image preloading has completed;\n" + stat;
            return stat;
        },
        get progress() {
        ...
        cropImage: function (img, c) {
            ...
            canvas.width = c[2];
            canvas.height = c[3];
            canvas.getContext('2d').drawImage(img, c[0], c[1], c[2], c[3], 0, 0, c[2], c[3]);
            return canvas;
        },
        /** @private */
        ...
        cutImages: function () {
            ...
            for (i in this.usrImages)
                if (this.usrImages.hasOwnProperty(i)) {
    ...
    */
    var Keyboard = function () {
        var keyName, codeNames = {}, keyCodes = {
            // Alphabet
        ...
        return declare('atom.Keyboard', {
            ...
            onKeyEvent: function (event, e) {
                var key = this.constructor.keyName(e), prevent = this.prevent(key);
                e.keyName = key;
                if (prevent)
                    e.preventDefault();
    ...
    */
    atom.trace = declare('atom.trace', {
        ...
        dumpRec: function dumpRec(obj, level, plain) {
            ...
            if (level > 5)
                return '*TOO_DEEP*';
            if (obj && typeof obj == 'object' && coreIsFunction(obj.dump))
                return obj.dump();
            function escape(v) {
    ...
    */
    atom.number = {
        ...
        between: function (number, n1, n2, equals) {
            ...
            n1 = Number(n1);
            n2 = Number(n2);
            return (n1 <= n2) && ((equals == 'L' && number == n1) ||
                (equals == 'R' && number == n2) ||
                (number > n1 && number < n2) ||
    ...
    atom.array = {
        ...
        range: function (from, to, step) {
            ...
            if (typeof from != 'number')
                throw new TypeError('`from` should be number');
            if (typeof to != 'number')
                throw new TypeError('`to` should be number');
         ...
         */
        sum: function (array) {
            for (var result = 0, i = array.length; i--;)
                result += array[i];
            return result;
        },
        /**
        ...
        product: function (array) {
            for (var result = 1, i = array.length; i--;)
                result *= array[i];
            return result;
        },
        /**
        ...
        associate: function (array, keys) {
            var i = 0, obj = {}, length = array.length, isFn = coreIsFunction(keys), keysSource = isFn ? array : keys;
            if (!isFn)
                length = Math.min(length, keys.length);
            for (; i < length; i++) {
        ...
        rgbToHex: function (array, asArray) {
            if (array.length < 3)
                return null;
            if (array.length == 4 && array[3] == 0 && !asArray)
                return 'transparent';
            var hex = [], i = 0, bit;
            for (; i < 3; i++) {
        ...
        reduce: function (array, callback, value) {
            if (coreIsFunction(array.reduce))
        ...
        reduceRight: function (array, callback, value) {
            if (coreIsFunction(array.reduceRight))
    ...
    */
    var prototypize = {
        ...
        proto: function (object, proto, methodsString) {
            coreAppend(object.prototype, atom.array.associate(methodsString.split(' '), proto));
            return prototypize;
        },
    ...
    */
    prototypize.add(function (globalObject) {
        ...
        if (!Array.prototype.reduce)
            Array.prototype.reduce = proto('reduce');
        if (!Array.prototype.reduceRight)
            Array.prototype.reduceRight = proto('reduceRight');
    });
    /*
    ---
    ...
    */
    (function () {
        var degree = Math.PI / 180, deg360 = Math.PI * 2;
        atom.math = {
            DEGREE360: deg360,
            /**
        ...
        atom.string = {
            ...
            replaceAll: function (string, find, replace) {
                if (toString.call(find) == '[object RegExp]') {
                    ...
                    for (var i in find)
                        string = this.replaceAll(string, i, find[i]);
                    return string;
                }
    ...
    */
    prototypize.add(function (globalObject) {
        prototypize.proto(globalObject.String, prototypize.fn(atom.string), 'safeHtml repeat substitute replaceAll contains begins ends ucfirst lcfirst');
    });
}.call(typeof exports == 'undefined' ? window : exports, Object, Array));
//# sourceMappingURL=atom-full-compiled.js.map 
//# sourceMappingURL=atom-full-compiled.js.map